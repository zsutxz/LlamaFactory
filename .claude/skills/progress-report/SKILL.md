...
```bash
cp .claude/skills/progress-report/config/sources.example.json config/sources.json
```
字段说明：
| 字段 | 必填 | 说明 |
|------|------|------|
...
2. **填凭据**：让用户把账号密码写进根目录 `.env`（环境变量名与 `credentials.*_env` 一致）。
   **Claude 不接触明文密码**；只核对变量名是否齐全。
3. **抓取+出报表**：在仓库根目录运行
   ```bash
   python .claude/skills/progress-report/scripts/fetch_progress.py \
