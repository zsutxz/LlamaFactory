...
| 3 | 偏好对齐 DPO | `stage: dpo` | chosen/rejected 对 | ✅ 已实测（三连负，见 §8） |
...
> ✅ 已实测（9B 三版实验，均未跑赢 SFT——结论与方法论见 §8，完整记录见 [train_list.md](./train_list.md) §13/§14）。配置蓝本：`examples/train_lora/qwen3_lora_dpo.yaml`。
...
> 完整实测与数据见 [train_list.md](./train_list.md) §13/§14。适用于任何"改数据/改超参→评测→再迭代"的实验循环。
...
> 且会部分冲掉定向 SFT 教的事实（4 条存留 2）。排序结论维持不变。详见 train_list §13.7。
