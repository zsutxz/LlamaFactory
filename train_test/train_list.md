...
3. DPO 真正起作用的前置：先把败局题对应条款补进 PT 语料/SFT QA（数据层扩量覆盖处罚条款与审批
   ...
   **数据覆盖 > PT > SFT >> DPO**。
4. **36 题净口径复算（08-25）**：剔除 3 道论文表格级不可答题（旧编号 6/11/30，留痕
   `data/domain_env_think_qa_removed.jsonl`；反查清单 `data/domain_env_gap_checklist.md`）后，
   v1 11/6/19 净胜 +5（仍不显著，p≈0.33）、v2 6/15/15、v3 7/16/13 均净负 9——**两轮"定向修复"
   实际把持平偏正的 v1 修成了明确负**。若重启 DPO，基线应回 v1 配置（sigmoid+原合成对）；
   同时稳输题败因统一为"编程序性细节/编列举"，与数据组成相关，见清单第三节。

### 13.7 v4（教训对照重试，2026-08-26）：打平收官

- **配方**（learning.md §8 六条纪律的落地）：v1 配方逐字复刻（sigmoid/beta0.1/lr5e-6/2ep）+
  数据 = 四类经典缺陷轮转 215 对（310 种子机械校验后；含 5 条定向 QA，其中太湖 3/黑土 1/
  水源 2 制成了偏好对）+ 基线换 SFT_t（§14 知识到位版，A/B 对照列 = SFT_t 自身答案）。
  不用 v2 无中生有、不用 v3 on-policy。产物 `pt_think_sft_then_dpo_v4`（215 对 54 步 2.4h，
  train_loss 0.354 属 sigmoid 正常拟合）。
- **结果（36 题三轮多数决）**：r1/r2/r3 = 10/12/14、10/12/14、11/11/14，**多数决 DPO 10 /
  SFT 12 / 平 14（净 -2，打平）**；三轮均分 2.81 vs 2.76。
- **判定**：
  1. 教训有效：v2/v3 的回归被修复（净 -9 → -2），"v1 配方 + 知识到位基线"是此数据量级
     DPO 的正确姿势；
  2. 但 DPO 仍无净增益：v1（+5）与 v4（-2）均在不显著带内，310 对量级上 DPO ≈ 无操作～
     微正，**维持定格 SFT**；
  3. 新发现——**DPO 会部分冲掉定向 SFT 教的事实**：4 条已教事实 DPO 后 2 条存留（水源罚款
     3.1→4.9 反升、黑土 5.0 保持），太湖米数 5.0→1.9 遗忘、陡坡梯田 5.0→3.0 半遗忘。
     存留与"该事实是否进了偏好对"无干净对应（太湖 3 对仍遗忘、黑土 1 对存留）——样本 4，
     只记为开放问题：若未来同时要"定向事实 + 偏好"，需专门验证偏好对能否保护定向教学。

## 14. 定向补强 SFT（根因验证，2026-08-25）

### 14.1 前提核查：语料全在，缺的是记忆

对 5 类缺口题逐条 grep PT 语料：**全部已覆盖**（各 1 块同块含关键事实，块索引
479/531/532/539/551）。所以缺口不是"没采集"而是"单次曝光记不住长尾数字"——修复走
B 线定向 QA 强化，不再补采集。

### 14.2 方法（单一变量：+5 条定向 QA）

- `generate_domain_qa.py` 新增 `--blocks` 参数（定向块索引出题，绕过随机梯子），
  5 块各出 1 题 → kimi 裁判 5/5 过筛（grounding 满分）→ `domain_env_qa_t_sft.jsonl`
  注册为 `domain_env_qa_t`；
- 重训 `qwen3_5_9b_domain_pt_then_sft_t.yaml`：dataset=305+5，其余超参与原 SFT
  逐字相同，从 PT 基线重训（非叠训），117 步 34.5 分钟，产物 `pt_think_then_sft_t`；
- 复评：36 题骨架副本（`domain_env_sftt_check.jsonl`）回填新答案 →
  `judge_domain_qa.py --pair ref-model` 单轮（reference 满分 5 做绝对标尺），
  基线对照 08-24 同模式的 `domain_env_onpolicy_report.md`。

### 14.3 结果：4/5 翻正，根因判定成立

| 旧# | 考点 | 旧 SFT | 新 SFT_t |
|---|---|---|---|
| 20 | 水源保护区排污口罚款 50-100 万 | 1.3 | **4.0** |
| 21 | 太湖禁止范围米数 | 1.3 | **5.0** |
| 25 | 黑土地剥离每㎡ 50-100 元 | 2.3 | **5.0** |
| 32 | 陡坡地退耕困难→修建梯田 | 2.3 | **5.0** |
| 1 | 核安全设备年报 **4 月 1 日**前 | 4.7 | 1.3（仍答 1 月 31 日） |

- **未中的那条是"同域相似帧"竞争**：语料 block 48 有"放射性物品运输容器编码清单
  每年 1 月 31 日前报上一年度"——同域同句式，先验帧压过单条 QA。对策：出 1-2 条
  **对比型 QA**（两条报送义务并排问）强化区分，而非再加单条重复。
- **整体均分 2.78 → 2.68（单轮裁判，−0.10 在噪声带内；升降题 12:17 偏负）**：
  重训整体重排了边缘题，定向收益与整体微降基本相抵。**SFT_t 未设为默认 adapter**
  （chat yaml 仍指原 SFT）：5 条事实对了但整体没涨，切换理由不足。
- 口径警示（yaml 头注也写了）：这 5 块与 eval 题同源，本轮复评目的是根因验证，
  分数不与 v1/v2/v3 DPO 对比口径混用。

### 14.4 结论

1. §13.6 判定闭环：**知识缺口是真因，数据层定向补强有效（4/5）**，偏好层三连负的
   解释链完整；
2. 定向补强的边界：无竞争帧的事实（数字/距离/措施）一条 QA 即中；有同域竞争帧的
   （相似报送日期）需对比型 QA；
3. 下一轮 SFT 扩量时把这 5 条并入常规 QA 池即可（`domain_env_qa_t` 已注册），
   若要修第 5 条，先补对比 QA 再一起重训，别单为一条再跑 35 分钟。
