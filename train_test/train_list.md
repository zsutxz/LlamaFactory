...
## 13. DPO v5 终验（2026-08-28）：五轮收官封存
...
### 13.8 v5（知识到位基线重启，2026-08-28）：§13.7 开放问题的答案是"否"

- **动机**：§13.7 留的开放问题——知识更到位的基线上，DPO 能否拿到 v4（净 -2）拿不到
  的增益。基线换 §15 的 `pt_think_then_sft_v5`（106 题净 +11）。
- **配方**：v1/v4 逐字复刻（sigmoid/beta0.1/lr5e-6/2ep/量化与 PT/SFT 全同）；数据
  `domain_env_pref_v5` 291 对（v4 的 215 对原样复用 + 76 新增：55 条新 QA 造对 + 当年
  劣化不合格种子 DeepSeek 非确定性二跑复检通过）。74 步 49 分钟（干净环境零抖动；
  占用环境同配方要 3.4h+），train_loss 0.3235，产物 `pt_think_sft_v5_then_dpo`。
- **结果（106 题三轮多数决，v5 口径）**：v5+DPO 29 / SFT_v5 31 / 平 46（**净 -2**）——
  与 v4 的净 -2 分毫不差。报告 `domain_env_qa_compare_v5dpo_report_majority.md`
  （编排 `train_test/run_eval_v5_dpo_ab.py`，judge 新增 `--pair v5-dpo`）。
- **交叉分析（与 v5 报告逐题对齐追踪）**：
  1. v5 净赢旧 SFT 的 31 题优势题：DPO 后守住 11 / **被冲掉 15** / 平 5——近半优势
     被 SFT→DPO 这一步自己冲掉；
  2. v5 净输的 20 题 DPO 仅救回 4；DPO 的 29 胜 = 11 守住 + 4 救回 + 14 来自原平题，
     增益与损耗对称抵消——**DPO 在此量级是重新洗牌，不是定向提升**；
  3. 罚则/审批类 23 题净 -1，其中 v5 优势题 4 条又被冲掉 2——与 §13.7 "4 条存留 2"
     同比例，50% 冲刷率第三次稳定复现（§13.7 → §15 → 本轮）。
- **判定：DPO 五轮收官封存**（v1 +5 / v2 -9 / v3 负 / v4 -2 / v5 -2，前四轮 36/39 题
  口径、本轮 106 题口径，净结论一致）：
...
## 15. v5 SFT 扩量（2026-08-27）：项目首个方向一致的净正增益

### 15.1 配方

- 数据：`domain_env_qa_v5` = `domain_env_qa_sft_v5.jsonl` **365 条** = §14 的 305 过筛池
  + 53 条罚则/审批定向过筛 + §14.4 的 5 条定向补强 + 2 条对比型 QA；
- 训练：`qwen3_5_9b_domain_pt_then_sft_v5.yaml`，从 PT 基线重训（非叠训），其余超参与
  §14 逐字相同；138 步 41.5 分钟，train_loss 0.8814，产物 `pt_think_then_sft_v5`；
- 评测：**106 题新口径**（旧 36 剔 5 污染 + 75 新留出过筛），三轮多数决；编排
  `train_test/run_eval_v5_ab.py`——双服务连续回填 + kimi 裁判奇偶换位 + 多数决汇总，
  任一环节失败显式抛错（服务 420s 未就绪/端口未释放/子脚本非零退出）。

### 15.2 结果：净 +11

- 多数决 SFT_v5 31 / SFT 20 / 平 55（**净 +11**，符号检验单侧 p=0.08）；r1/r2/r3 单轮
  净 +3/+2/+2 全正——四轮 DPO 从未出现过的方向一致性；
- 总均分 2.80 vs 2.80 持平：增益是具体题换赢，不是整体提分；
- 报告：`domain_env_qa_compare_v5_report_majority.md`。
...
- `pt_think_then_sft_v5` 已作为后续实验的 SFT 基线（§13.8 的 DPO 基线即此）；
